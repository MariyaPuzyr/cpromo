$(document).ready(function(){
    ...
    $("[id^='buyCoins']").on('click', function(){
        $("#hidescreen, #loadingData").fadeIn(10);
        $("#modalData").load('/finance/buyCoins', function(){
            $.fancybox.open($("#modalWindow"));
            $("#hidescreen, #loadingData").fadeOut(10);
        });
    });
    ...
    $("#terms_agree_register").on('click', function(){
        $("#hidescreen, #loadingData").fadeIn(10);
        $("#modalData").load('/info/terms/?partial=1', function(){
            $.fancybox.open($("#modalWindow"));
            $("#hidescreen, #loadingData").fadeOut(10);
        });
    });
    
    $("#Users_googleAuth").on('change', function(){
        var state = $(this).prop("checked")? 1 : 0;
        
        $.ajax({
            url: '/user/profile/enableGoogle?state='+state,
            type: 'get',
            cache: false,
            success: function(html){  
                if(state == 1) {
    ...
    $("#btn_order_sell").on('click', function(){
        $("#modalData").load('/exchange/orderSell', function(){
            $.fancybox.open($("#modalWindow"));
        });
    });
